...
class Widget extends DmYY {
  ...
  init = async () => {
    try {
      ...
      this.fee.iconColor = logoColor
        ? new Color(logoColor)
        : this.fee.iconColor;
      this.flow.iconColor = flowIconColor
        ? new Color(flowIconColor)
        : this.flow.iconColor;
      this.voice.iconColor = voiceIconColor
        ? new Color(voiceIconColor)
        : this.voice.iconColor;
      ...
      if (userInfo.code === "Y") {
        ...
        userInfo.data.dataList.forEach((item) => {
          ...
          if (item.type === "flow") {
            ...
            this.flow.percent = item.persent
              ? (100 - item.persent).toFixed(2)
              : this.settings.flow
              ? ((this.flow.number / this.settings.flow) * 100).toFixed(2)
              : 100;
          ...
          if (item.type === "voice") {
            ...
            this.voice.percent = item.persent
              ? (100 - item.persent).toFixed(2)
              : this.settings.voice
              ? ((this.voice.number / this.settings.voice) * 100).toFixed(2)
              : 100;
    ...
    return this.renderAppView([
      ...
      },
      {
        menu: [
          {
            icon: {
              name: "antenna.radiowaves.left.and.right.circle",
              color: "#448EF7",
            },
            type: "color",
            title: "流量进度条",
    ...
    return this.renderAppView([
      ...
      },
      {
        menu: [
          ...
          {
            icon: {
              name: "rectangle.lefthalf.inset.fill.arrow.left",
              color: "#7DD35F",
            },
            type: "input",
            title: "中尺寸组件边距",
      ...
      this.registerAction({
        ...
        menu: [
          ...
          },
          {
            name: "size",
            icon: {
              name: "arrow.down.backward.and.arrow.up.forward.square",
              color: "#F77F29",
            },
            title: "尺寸设置",
            type: "input",
