...
Object.keys(doubleDate).forEach(function (day) {
  doubleDay.push(day);
  });
...
async function setSmallBaitiaoShow(widget) {
  ...
  const baitiaoValue = oneStack.addText(baitiaoAmount);
  baitiaoValue.font = Font.mediumSystemFont(15 * size.SC);
  oneStack.addSpacer();
  widget.addSpacer(5 * size.SC);
  const twoStack = widget.addStack();
  twoStack.centerAlignContent();
  const baitiaoImage = twoStack.addImage(await getImage(baitiaoImg));
  baitiaoImage.imageSize = new Size(127 * 0.17 * size.SC, 75 * 0.17 * size.SC);
  twoStack.addSpacer(5 * size.SC);
  const baitiaoText = twoStack.addText(baitiaoTitle);
  baitiaoText.font = Font.regularSystemFont(13 * size.SC);
  twoStack.addSpacer();
...
async function createChart() {
  ...
  let topPadding = size.chartTopPadding;
  if (config.widgetFamily == 'small') {
    data.splice(0, 2);
    labels.splice(0, 2);
...
async function getBaitiaoData() {
  ...
  /*
  if(res.resultCode === 0) {
    console.log("白条信息获取成功");
  } else {
    console.log("白条信息获取失败，请在脚本内关闭白条显示");
  };
  */
  return res;
}
