...
class Widget extends DmYY {
  ...
  setList(stack, data) {
    ...
    const value = valueStack.addText(data.number.toString());
  ...
  async checkAndUpdateScript() {
    ...
    const latestVersion = response.find(i => i.name === "ChinaBroadnet_2024").version;
    const downloadUrl = response.find(i => i.name === "ChinaBroadnet_2024").downloadUrl;
  ...
  renderMedium = async (w) => {
    ...
    const canvas = this.makeCanvas();
    const bodyStack = w.addStack();
    await this.mediumCell(canvas, bodyStack, this.fee, '3F85AD', true);
