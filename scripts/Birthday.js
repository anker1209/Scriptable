...
getAge = (beginStr) => {
  ...
  ageYear = tmpBirth.year > 0 ? `${tmpBirth.year}年` : '';
  ageMonth = tmpBirth.month > 0 ? `${tmpBirth.month}月` : '';
  ageDay = tmpBirth.day > 0 ? `${tmpBirth.day}天` : '';
...
class Widget extends DmYY {
  ...
  setRightView = (right) => {
    ...
    this.setRightCell(right, countIcon, "1ab6f8", `年龄`, getAge(this.defaultData.eday));
